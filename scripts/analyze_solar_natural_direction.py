...
INPUT = Path("outputs/mechanism_replication/natural_direction/solar")
OUTPUT = Path("outputs/mechanism_replication/analysis/natural_direction/solar")
...
def main() -> None:
    ...
    verdict = "PENDING INTERPRETATION FROM SAVED A/N/C RESULTS"
    summary = {"dataset": "Solar", "evaluation": "fixed archived aligned 28,006-row readout; exact L1 decisions", "per_objective": summaries, "verdict": verdict, "no_backbone_training": True, "no_head_fitting": True}
    (OUTPUT / "summary.json").write_text(json.dumps(summary, indent=2) + "\n")
    (OUTPUT / "analysis_manifest.json").write_text(json.dumps({"script": "scripts/analyze_solar_natural_direction.py", "sources": sources, "timestamp_utc": datetime.now(timezone.utc).isoformat(), "no_backbone_training": True, "no_head_fitting": True}, indent=2) + "\n")
