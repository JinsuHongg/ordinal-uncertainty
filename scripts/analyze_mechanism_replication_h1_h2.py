#!/usr/bin/env python3
"""Frozen RetinaMNIST confirmatory H1/H2a analysis from completed A/C artifacts.

This analysis is read-only with respect to the completed A/C condition outputs:
it neither trains a backbone/head nor evaluates any Solar or UTKFace artifact.
"""
...
INPUT_ROOT = Path("outputs/mechanism_replication/ac/retina")
OUTPUT_ROOT = Path("outputs/mechanism_replication/analysis/retina")
SEEDS = (1, 2, 3, 4)
CLASS4 = 4
...
def h1_delta(a_l1: np.ndarray, c_l1: np.ndarray, labels: np.ndarray) -> dict[str, float | int]:
    """Compute the frozen H1 effect as C MAE minus A MAE on true class 4."""
    mask = np.asarray(labels) == CLASS4
    if int(mask.sum()) != 66:
        raise ValueError(f"expected 66 class-4 samples, found {int(mask.sum())}")
...
def load_condition(objective: str, seed: int) -> tuple[dict[str, object], dict[str, np.ndarray], Path]:
    root = INPUT_ROOT / objective / f"seed_{seed}"
    ...
    if manifest["dataset"] != "retina" or manifest["backbone_objective"] != objective or manifest["backbone_seed"] != seed:
    ...
    if arrays["labels"].shape != (1080,) or int((arrays["labels"] == CLASS4).sum()) != 66:
        raise ValueError(f"unexpected Retina class-4 population: {root}")
...
def analyze_objective(objective: str, output: Path) -> dict[str, object]:
    ...
    sources: list[str] = []
    for seed in SEEDS:
        manifest, arrays, root = load_condition(objective, seed)
        sources.append(str(root))
        h1 = h1_delta(arrays["a_l1"], arrays["c_l1"], arrays["labels"])
        mask = arrays["labels"] == CLASS4
        h1.update({
            "objective": objective,
            "seed": seed,
    ...
    manifest = {
        "specification": SPECIFICATION, "dataset": "retina", "objective": objective,
...
def main() -> None:
    ...
    parser.add_argument("--dataset", choices=("retina",), default="retina")
    parser.add_argument("--objective", choices=("ce", "rps", "both"), default="both")
    parser.add_argument("--output-root", type=Path, default=OUTPUT_ROOT)
    args = parser.parse_args()
    if args.output_root.exists():
        raise FileExistsError(f"refusing to overwrite completed analysis output: {args.output_root}")
    objectives = ("ce", "rps") if args.objective == "both" else (args.objective,)
    results = {objective: analyze_objective(objective, args.output_root / objective) for objective in objectives}
    combined = {
    ...
    (args.output_root / "retina_confirmatory_summary.json").write_text(json.dumps(combined, indent=2) + "\n", encoding="utf-8")
