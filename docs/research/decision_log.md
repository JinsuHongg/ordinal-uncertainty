...
| Phase 3.19 RetinaMNIST sampling/objective disentanglement | A — BALANCED SAMPLING IS THE PRIMARY DIRECTION-ADAPTATION DRIVER | Complete | In frozen-RPS, training-only five-fold OOF direction-only heads, balancing improves C4 MAE/shrinkage/margin under both CE and RPS, while objective contrasts within sampling strata are small. This is bounded to the tested RetinaMNIST protocol and does not select a correction or establish universal causality. |
| Phase 3.20A RetinaMNIST imbalance-severity dose response | B — PARTIAL / NON-MONOTONIC IMBALANCE EFFECT | Complete | In the fixed 25-run CE grid, lower C4 support consistently reduces p4 and worsens the C4-vs-C3 margin, but MAE, shrinkage, severe burden, and centroid routing are non-monotonic and seed-variable. Do not strengthen the project phenomenon into a universal dose-response claim. |
| RetinaMNIST natural-sampling direction-only follow-up | C — BALANCED SAMPLING IS PRIMARY | Complete | Across confirmatory CE/RPS seeds 1–4, natural N improves endpoint MAE in only 2/4 seeds per objective and is worse than balanced C in every seed. This is descriptive Retina-only evidence; it changes neither frozen H1/H2a nor Solar status. |
| Solar RPS natural-sampling direction-only follow-up | D — MIXED | Complete | On validated RPS frozen features, N improves X exact-L1 MAE in 3/4 seeds but is weaker than C in every seed. N does not reproduce C's broad endpoint-mass redistribution and has a smaller global/severe cost. |
| Solar CE seed-2 replacement | B — replacement A/C/N complete | Complete | Original seed 2 remains archived after its feature-identity failure. One pre-declared end-to-end replacement restores replacement-aware A/C H1/H2a with 4/4 negative H1 deltas and strong H2a; replacement N improves A but remains weaker than C. Its contemporaneous cross-seed limitation was superseded after the separately pre-specified N fits for seeds 1, 3, and 4 completed. |
| Solar CE natural-sampling direction-only follow-up | C — NOT CONSISTENT | Complete | Validated original CE features for seeds 1, 3, and 4 plus the coherent replacement seed 2 yield N improvement in 2/4 units and N worse than balanced C in all four. N's endpoint-mass shift is small and upper-concentrated relative to C's broad redistribution. Auxiliary descriptive evidence only; frozen H1/H2a and manuscript claims are unchanged. |
| Paper story architecture | Mechanism-paper claim and figure plan frozen for audit | Active planning | The manuscript is organized around the cross-dataset phenomenon, mixed representation/head failure, direction responsiveness, RetinaMNIST sampling source, and the severity boundary. The next authorized task is source-artifact/data-availability audit, not a new experiment or final plotting. |
